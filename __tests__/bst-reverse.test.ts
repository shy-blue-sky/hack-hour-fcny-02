...
describe('bstReverse', () => {
  ...
  it('returns the input root node', () => {
    bst = new BinarySearchTree(1);
    const result = bstReverse(bst);
    expect(result).toBe(bst); // Should return same object reference
  });
});

describe.skip('sortedArrayToBST', () => {
  let bstResult: BinarySearchTree | null;
  let bstTest: BinarySearchTree | null;
