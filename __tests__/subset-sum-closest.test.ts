...
} from '../challenges/subset-sum-closest';

describe('subsetSumClosest', () => {
  let arr: number[];
...
describe.skip('generateCombinations', () => {
  const normalize = (subsets: number[][]) => subsets.sort();
  let arr: number[], solution: number[][];

  it('handles cases that sum up to target', () => {
    arr = [2, 3, 6, 7];
    solution = [[7], [2, 2, 3]];
    expect(normalize(generateCombinations(arr, 7))).toEqual(
      normalize(solution)
    );
    ...
    expect(normalize(generateCombinations(arr, 8))).toEqual(
      normalize(solution)
    );
  });
  ...
  it("handles cases that don't sum up to target", () => {
    arr = [3, 4, 7, 8, 11];
    solution = [];
    expect(normalize(generateCombinations(arr, 5))).toEqual(
      normalize(solution)
    );
  });
});
