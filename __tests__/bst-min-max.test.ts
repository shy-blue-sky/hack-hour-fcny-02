...
import { BinarySearchTree, bstMinMax, lowestCommonAncestor } from '../challenges/bst-min-max';
...
describe.skip('lowestCommonAncestor', () => {
  ...
  it('calculates ancestors of binary search tree nodes with p and q ascending in value', () => {
    ...
    expect(lowestCommonAncestor(bst, bst.left!.left!, bst.left!.right!)).toBe(
      bst.left
    );
    expect(
      lowestCommonAncestor(bst, bst.right!.right!.left!, bst.right!.right!)
    ).toBe(bst.right!.right);
    expect(lowestCommonAncestor(bst, bst.left!, bst.right!.right!.left!)).toBe(
      bst
    );
  });
  ...
  it('calculates ancestors of binary search tree nodes with p and q not ascending in value', () => {
    ...
    bst.right.right = new BinarySearchTree(9);
    bst.right.right.left = new BinarySearchTree(8);
    expect(lowestCommonAncestor(bst, bst.left!.right!, bst.left!.left!)).toBe(
      bst.left
    );
    expect(
      lowestCommonAncestor(bst, bst.right!.right!, bst.right!.right!.left!)
    ).toBe(bst.right!.right);
    expect(lowestCommonAncestor(bst, bst.right!.right!.left!, bst.left!)).toBe(
      bst
    );
    expect(lowestCommonAncestor(bst, bst.left!.right!, bst.left!.right!)).toBe(
      bst.left!.right
    );
  });
});
